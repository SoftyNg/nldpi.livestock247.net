...
<div class="container-fluid">
  ...
  <div class="row">
      ...
      <div class="card-body">
        ...
        <div class="collapse show" id="filterOptions">
          <div class="custom-control custom-radio mb-2">
            ...
            <label class="custom-control-label font-weight-bold" for="allServices">
              All Services <span class="text-muted font-weight-normal">(<?= count($all_services); ?>)</span>
            </label>
          </div>

          <div class="custom-control custom-radio mb-2">
            <input type="radio" id="vetProfessionals" name="serviceFilter" class="custom-control-input">
            <label class="custom-control-label" for="vetProfessionals">
              Vet Professionals <span class="text-muted">(<?= count($vets); ?>)</span>
            </label>
          </div>

          <div class="custom-control custom-radio mb-2">
            <input type="radio" id="healthServices" name="serviceFilter" class="custom-control-input">
            <label class="custom-control-label" for="healthServices">
              Health Services <span class="text-muted">(<?= count($health); ?>)</span>
            </label>
          </div>

          <div class="custom-control custom-radio mb-2">
            <input type="radio" id="markets" name="serviceFilter" class="custom-control-input">
            <label class="custom-control-label" for="markets">
              Markets <span class="text-muted">(<?= count($markets); ?>)</span>
            </label>
          </div>

          <div class="custom-control custom-radio mb-2">
            <input type="radio" id="farmersKeepers" name="serviceFilter" class="custom-control-input">
            <label class="custom-control-label" for="farmersKeepers">
              Farmers/Keepers <span class="text-muted">(<?= count($keepers); ?>)</span>
            </label>
          </div>

          <div class="custom-control custom-radio mb-2">
            <input type="radio" id="transporters" name="serviceFilter" class="custom-control-input">
            <label class="custom-control-label" for="transporters">
              Transporters <span class="text-muted">(<?= count($transporters); ?>)</span>
            </label>
          </div>
        </div>
      </div>
    ...
    <div class="col-md-8" style="background-color:#F5FCF9;">
      <div id="serviceList">
        <?php
          json($all_services); // For debugging, you can remove this in production
...
<script>
  $(document).ready(function () {
    ...
    // Load default (all) services on page load
    fetchFilteredServices();
  });
</script>
